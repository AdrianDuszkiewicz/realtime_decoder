...
from realtime_decoder import base
...
from realtime_decoder import (
    datatypes, position, oenet, stimulation,
    main_process, ripple_process, encoder_process,
    decoder_process, gui_process, base, messages,
    merge_rec
)
from realtime_decoder.stimulation import DummyStimDecider
# from line_profiler import LineProfiler
# run with e.g.:
# mpiexec -np 5 python -u runscript_replay.py config_replay.yaml
...
def setup(config_path, numprocs):
    ...
    if rank in config['rank']['supervisor']:
        ...
        lfp_interface = oenet.OEDataReceiver(
            comm, rank, config, datatypes.Datatypes.LFP
        )
        pos_interface = oenet.OEDataReceiver(
        ...
        spikes_interface = oenet.OEDataReceiver(
